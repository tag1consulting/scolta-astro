 ...
 * content-change events (a CMS save webhook, a Nitro/SSR route, a watcher).
 *
 ...
 * deploy) has no shared in-process timer across invocations — trigger rebuilds
 * via webhook/CI there instead. This mirrors the {@link ScoltaTracker} shipped
 * by scolta-next, whose Payload `afterChange`/`afterDelete` hooks are a
 * reference wiring; bulk writes that bypass the hooks need a manual
 * `npx scolta-build`.
 */
...
export class ScoltaTracker {
  ...
  constructor(
    private readonly config: AstroScoltaConfig,
