/**
 * Index build runner for both Astro content modes.
 *
 * The crawl path is framework-agnostic — it walks rendered HTML and does not
 * care that Astro (not Next/Nuxt) produced it — so this is the same thin glue
 * over the shared `scolta` binding's in-process indexer, with `astro build`'s
...
/** Map a build-output-relative HTML path to the URL Astro serves it at. */
