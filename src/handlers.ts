/**
 * Framework-agnostic AI endpoint logic shared by the injected Astro routes.
 *
 * Returns the binding's plain EndpointResult objects; the route modules in
 * `routes/` wrap these with Web `Request`/`Response` handling. All orchestration
 * is the SAME `ai.AiEndpointHandler` the Next/Nuxt adapters use — no logic is
 * duplicated here.
 */

import { ai, type CacheDriver, NullCacheDriver, HealthChecker } from "scolta";
import type { AstroScoltaConfig } from "./config.js";
 ...
 */
function defaultAiService(config: AstroScoltaConfig): ai.AiServiceLike {
...
export function createScoltaApi(config: AstroScoltaConfig, opts: ScoltaApiOptions = {}): ScoltaApi {
