...
// Measured unpacked size of the current good artifact: 193084 bytes
// (~0.184 MB), 59 files, `npm pack --dry-run --json` on 2026-06-14. Cap at
// ~2x the measured value so routine growth (more dist chunks, additional
// route subpaths) passes but a runaway (bundled fixtures, a stray vendored
// WASM/binary, the 13 MB-zip class of mistake) trips the guard.
const MAX_UNPACKED_BYTES = 400_000; // ~2x measured 193,084

// Derive the allowlist of packed-path prefixes from `files`. For an Astro
// integration this legitimately covers `dist/` (the integration, /core,
// /build, the injected `routes/*` entrypoints, the `scolta-build` CLI, the
// bootstrap, and their .d.ts/.d.cts/.map siblings), plus the root-level
// `ScoltaSearch.astro` mount component and the doc/license files. `files`
// entries are bare names ("dist", "README.md"): a directory entry matches
// that path or anything beneath it; a file entry matches exactly. npm ALSO
// always includes package.json regardless of `files`, so it is allowed
// unconditionally.
