...
describe("the Amazee opt-in gate", () => {
  it("introduces no provider of its own, from an object or from the env", () => {
    ...
    // silently completed.
    const bare = AstroScoltaConfig.fromObject({}).scolta.ai_provider;

    expect(AstroScoltaConfig.fromEnv({}, {}).scolta.ai_provider).toBe(bare);
    expect(
      AstroScoltaConfig.fromEnv({}, { SCOLTA_API_KEY: "sk-env" }).scolta.ai_provider,
    ).toBe(bare);
  });

  it("takes ai_provider = amazee from the environment as the opt-in", () => {
    const config = AstroScoltaConfig.fromEnv({}, { SCOLTA_AI_PROVIDER: "amazee" });

    expect(config.scolta.ai_provider).toBe("amazee");
  });

  it("makes no Amazee call on a request path when no provider is selected", async () => {
    const api = createScoltaApi(AstroScoltaConfig.fromObject({ outputDir: tmp, stateDir: tmp }));
  ...
  it("makes no Amazee call on a request path for a non-Amazee provider", async () => {
    const api = createScoltaApi(
      AstroScoltaConfig.fromObject({ outputDir: tmp, stateDir: tmp, ai_provider: "anthropic" }),
    );
  ...
  it("provisions nothing on a health check, whatever the provider", async () => {
    for (const ai_provider of ["", "anthropic", "amazee"]) {
      const api = createScoltaApi(AstroScoltaConfig.fromObject({ outputDir: tmp, stateDir: tmp, ai_provider }));
