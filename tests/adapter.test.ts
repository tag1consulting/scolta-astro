/** scolta-astro adapter: config round-trip, build modes, handler logic, bootstrap. */
...
import { AstroScoltaConfig } from "../src/config.js";
...
let tmp: string;
beforeEach(() => {
  tmp = fs.mkdtempSync(path.join(os.tmpdir(), "scolta-astro-"));
});
afterEach(() => {
...
describe("config round-trip (Release Gate family 4)", () => {
  it("browser config reflects SAVED values", () => {
    const config = AstroScoltaConfig.fromObject({ site_name: "Astro Site", results_per_page: 9 });
    const b = config.toBrowserConfig();
    expect(b["siteName"]).toBe("Astro Site");
    expect((b["scoring"] as any)["RESULTS_PER_PAGE"]).toBe(9);
  });

  it("fromEnv reads SCOLTA_API_KEY", () => {
    const c = AstroScoltaConfig.fromEnv({}, { SCOLTA_API_KEY: "sk-env" } as any);
    expect(c.scolta.ai_api_key).toBe("sk-env");
  });

  it("default output dir is the astro build target", () => {
    expect(new AstroScoltaConfig().exportDir).toBe("dist");
    expect(new AstroScoltaConfig().outputDir).toBe("dist");
  });
});
...
describe("static-output crawl + build", () => {
  ...
  it("crawls rendered HTML and builds a valid index", async () => {
    ...
    const config = AstroScoltaConfig.fromObject({
...
describe("content-source build hits the token cache", () => {
  it("second build re-tokenizes zero unchanged items", async () => {
    ...
    const config = AstroScoltaConfig.fromObject({ source: "content", outputDir: path.join(tmp, "out"), stateDir: path.join(tmp, "state") });
...
describe("AI handler logic (createScoltaApi)", () => {
  ...
  it("expandQuery returns ok + terms", async () => {
    const api = createScoltaApi(AstroScoltaConfig.fromObject({}), { aiService: fakeService('["t1","t2","t3"]'), logger: silent });
  ...
  it("followUp validates messages", async () => {
    const api = createScoltaApi(AstroScoltaConfig.fromObject({}), { aiService: fakeService("reply"), logger: silent });
  ...
  it("health is status-only by default — monitors get the status, no diagnostics", async () => {
    const api = createScoltaApi(AstroScoltaConfig.fromObject({ results_per_page: 21 }));
  ...
  it("health reflects saved scoring when healthDetail is enabled", async () => {
    const api = createScoltaApi(AstroScoltaConfig.fromObject({ results_per_page: 21, healthDetail: true }));
    const h = await api.health();
...
describe("buildWindowScolta", () => {
  it("reflects config and derives the wasmPath glue module (never the directory)", () => {
    const config = AstroScoltaConfig.fromObject({ site_name: "Acme" });
    ...
    expect(String(win["wasmPath"]).endsWith("/wasm/scolta_core.js")).toBe(true);
  });
