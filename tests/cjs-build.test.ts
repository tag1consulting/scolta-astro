...
function runCli(entry: string): { dir: string; stdout: string; status: number | null } {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "scolta-astro-cli-"));
  ...
  it.skipIf(entry === "cli.cjs" && !scoltaCjsWorks)(
    ...
    () => {
      ...
      try {
        ...
        // the astro build target (dist).
        expect(stdout).toContain("Copied");
        expect(fs.existsSync(path.join(dir, "dist", "scolta", "js", "scolta.js"))).toBe(true);
